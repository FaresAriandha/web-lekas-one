...
@section('content')
    <div class="bg-white shadow rounded-2xl p-6 flex flex-col" id="container-table">
        ...
        <form class="w-full flex justify-between items-start mb-4 flex-col sm:flex-col"
            action="{{ route('admin.locations.store') }}" method="post">
            @csrf
            {{-- Baris 1 --}}
            <div
                ...
                <div class="w-full">
                    ...
                    <div class="relative">
                        <input type="text" id="shploc_name" aria-describedby="helper-text-explanation"
                            ...
                            maxlength="100" value="{{ old('shploc_name') }}">
                        @error('shploc_name')
                ...
                <div class="w-full">
                    ...
                    <div class="relative">
                        <textarea type="text" id="shploc_address" aria-describedby="helper-text-explanation"
                            ...
                            placeholder="Jl. Lorem Ipsum No. 255" name="shploc_address" autocomplete="off" data-char-count maxlength="255">{{ old('shploc_address') }}</textarea>
                        ...
                        <select
                            ...
                            @foreach ($prices as $price)
                                <option value="{{ $price->spl_ID }}"
                                    {{ old('spl_ID') == $price->spl_ID ? 'selected' : '' }}>
                                    {{ $price->spl_name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="w-full">
                    ...
                    <div class="relative">
                        <textarea type="text" id="shploc_url_maps" aria-describedby="helper-text-explanation"
                            class="border-0 ring-1 ring-[#344357] text-[#344357] text-sm rounded-lg focus:ring-2 focus:outline-none block w-full p-2.5 resize-y"
                            placeholder="https://www.google.com/maps?q=-6.2088,106.8456" name="shploc_url_maps">{{ old('shploc_url_maps') }}</textarea>
