...
@section('content')
    <div class="bg-white shadow rounded-2xl p-6 relative" id="container-table">
        ...
        <div class="flex justify-between items-start sm:items-start mb-4 flex-col sm:flex-col">
            <h2 class="text-xl font-semibold mb-10">Daftar Kurir</h2>
                        ...
                        @foreach ($couriers as $index => $courier)
                            @php
                                ...
                                if ($diff->y > 0 && $diff->m == 0) {
                                    ...
                                    $formattedDiff = "{$diff->y} tahun";
                                } elseif ($diff->y > 0 && $diff->m > 0) {
                                    // Jika ada tahun dan bulan
                                    $formattedDiff = "{$diff->y} tahun {$diff->m} bulan";
                                } else {
                                    // Jika hanya bulan
                                    $formattedDiff = "{$diff->m} bulan";
                                }
                            @endphp
                            <tr class="bg-white border-b border-gray-200 hover:bg-gray-100">
                                <th scope="row" class="px-6 py-4 font-medium text-center">
                                    {{ $couriers->firstItem() + $index }}
                                </th>
            ...
            <div class="justify-between items-center mt-6 flex">
                <div>
                    <h1 class="text-xs sm:text-base">
                        Showing {{ $couriers->firstItem() }} to {{ $couriers->lastItem() }} of {{ $couriers->total() }}
                        entries
                    </h1>
                </div>
                <div class="space-x-1">
