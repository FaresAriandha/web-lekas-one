    ...
    <div class="bg-white shadow rounded-2xl p-6" id="container-table">
        ...
        <div class="flex justify-between items-start sm:items-start mb-4 flex-col sm:flex-col">
            ...
            <div class="flex flex-col sm:flex-row items-start flex-wrap justify-between w-full ">
                <form class="flex flex-col sm:flex-row items-start sm:items-center flex-wrap" method="get"
                    action="{{ route('admin.assignees.index') }}">
                    {{-- Dropdown Status --}}
                    <div
                        ...
                        <select
                            ...
                            <option value="Ditugaskan"
                                {{ isset($assignee_status) && $assignee_status == 'Ditugaskan' ? 'selected' : '' }}>
                                Ditugaskan
                            </option>
                            <option value="Siap Pickup"
                                {{ isset($assignee_status) && $assignee_status == 'Siap Pickup' ? 'selected' : '' }}>
                                Siap Pickup
                            </option>
                            <option value="Dalam Tugas"
                                {{ isset($assignee_status) && $assignee_status == 'Dalam Tugas' ? 'selected' : '' }}>Dalam
                                Tugas
                            </option>
                            <option value="Selesai"
                                {{ isset($assignee_status) && $assignee_status == 'Selesai' ? 'selected' : '' }}>Selesai
                            </option>
                        </select>
                    ...
                    <div class="relative max-w-sm mb-[10px] sm:mb-0 sm:mr-[10px]">
                        <input id="datepicker-autohide" datepicker datepicker-autohide datepicker-format="yyyy-mm-dd"
                            ...
                            placeholder="Pilih tanggal" name="assignee_date" autocomplete="off"
                            value="{{ isset($assignee_date) ? $assignee_date : '' }}">
                ...
                <a href="{{ route('admin.assignees.create') }}"
                ...
                <tbody class="text-[15px] h-fit text-black text-nowrap">
                    @if (count($assignees) > 0)
                        @foreach ($assignees as $index => $assignee)
                            @php
                                $status = $assignee->cas_status;

                                $colorClass = match ($status) {
                                    'Ditugaskan' => 'bg-gray-600',
                                    'Siap Pickup' => 'bg-blue-600',
                                    'Dalam Tugas' => 'bg-amber-600',
                            ...
                            <tr class="bg-white border-b border-gray-200 hover:bg-gray-100 sm:text-left text-center">
                                ...
                                <td class="px-6 py-4">
                                    {{ $assignee->courier->courier_name }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $assignee->courier->fleet->fleet_nopol }}
                                </td>
                                <td class="px-6 py-4 text-center">
                                    @if ($assignee->cas_type == 'pasjay')
        ...
        @if (count($assignees) > 0)
            <div class="justify-between items-center mt-6 flex">
                ...
                <div class="space-x-1">
                    @if ($assignees->currentPage() > 1)
                        <a href="{{ $assignees->previousPageUrl() }}"
                    ...
                    @if ($assignees->hasMorePages())
                        <a href="{{ $assignees->nextPageUrl() }}"
                            class="px-4 py-2 ring-1 ring-black rounded-lg hover:bg-gray-100 cursor-pointer">
