...
@section('content')
    <div class="bg-white shadow rounded-2xl p-6 relative" id="container-table">
        ...
        <div class="flex justify-between items-start sm:items-start mb-4 flex-col sm:flex-col">
            <h2 class="text-xl font-semibold mb-10">Daftar Armada</h2>
            <div class="flex flex-col sm:flex-row items-start flex-wrap justify-between w-full ">
                <form class="flex flex-col sm:flex-row items-start sm:items-center flex-wrap" method="get"
                    action="{{ route('admin.fleets.index') }}">
                    {{-- Dropdown Status --}}
                    <div
                        ...
                        <select
                            ...
                            <option value="TERSEDIA"
                                {{ isset($fleet_status) && $fleet_status == 'TERSEDIA' ? 'selected' : '' }}>
                                TERSEDIA
                            </option>
                            <option value="DIGUNAKAN"
                                {{ isset($fleet_status) && $fleet_status == 'DIGUNAKAN' ? 'selected' : '' }}>
                                DIGUNAKAN
                            </option>
                            <option value="PERBAIKAN"
                                {{ isset($fleet_status) && $fleet_status == 'PERBAIKAN' ? 'selected' : '' }}>PERBAIKAN
                            </option>
                        </select>
        ...
        <div class="relative overflow-x-scroll no-scrollbar shadow-md rounded-lg">
            <table class="w-full h-fit text-sm text-left rtl:text-right text-gray-500">
                ...
                <tbody class="text-[15px] h-fit text-black text-nowrap sm:text-left text-center">
                    @if (count($fleets) > 0)
                        @foreach ($fleets as $index => $fleet)
                            <tr class="bg-white border-b border-gray-200 hover:bg-gray-100">
                                <th scope="row" class="px-6 py-4 font-medium text-center">
                                    {{ $fleets->firstItem() + $index }}
                                </th>
                                <td class="px-6 py-4">
                                    {{ $fleet->fleet_nopol }}
                                </td>
                                ...
                                <td class="px-6 py-4 text-center">
                                    @if ($fleet->fleet_status == 'TERSEDIA')
                                        <span class="p-2 rounded-lg bg-green-800 text-white font-semibold">
                                            {{ $fleet->fleet_status }}
                                        </span>
                                    @elseif ($fleet->fleet_status == 'DIGUNAKAN')
                                        <span class="p-2 rounded-lg bg-[#52C3BE] text-white font-semibold">
                                            {{ $fleet->fleet_status }}
                                        </span>
                                    @else
                                        <span class="p-2 rounded-lg bg-red-600 text-white font-semibold">
                                            {{ $fleet->fleet_status }}
                                        </span>
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ $fleet->courier->courier_name ?? '-' }}
                                </td>
                                <td class="px-6 py-4 flex justify-center">
                                    <div class="relative w-fit h-fit">
                                        ...
                                        <div
                                            class="menu-popup absolute {{ $fleets->firstItem() + $index == $fleets->lastItem() ? 'bottom-0' : '-top-[10px]' }} -left-[140px] hidden sm:right-36 w-32 bg-white shadow-lg rounded-md z-50 p-2 text-nowrap text-center">
                                            <a href="{{ route('admin.fleets.show', $fleet->fleet_ID) }}"
                                                class="block px-4 py-2 text-gray-700 hover:bg-gray-100 active:bg-gray-100 focus:bg-gray-100 rounded-md">Lihat
                                                Detail</a>
                                            <a href="{{ route('admin.fleets.edit', $fleet->fleet_ID) }}"
                                                class="block px-4 py-2 text-gray-700 hover:bg-gray-100 active:bg-gray-100 focus:bg-gray-100 rounded-md">Edit
                                                Data</a>
                                            <button type="button" data-url="/admin/fleets/{{ $fleet->fleet_ID }}"
            ...
            <div class="justify-between items-center mt-6 flex">
                <div>
                    <h1 class="text-xs sm:text-base">
                        Showing {{ $fleets->firstItem() }} to {{ $fleets->lastItem() }} of {{ $fleets->total() }}
                        entries
                    </h1>
                </div>
                <div class="space-x-1">
                    @if ($fleets->currentPage() > 1)
                        <a href="{{ $fleets->previousPageUrl() }}"
