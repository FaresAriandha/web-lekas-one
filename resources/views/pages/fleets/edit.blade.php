...
@section('content')
    <div class="bg-white shadow rounded-2xl p-6 flex flex-col" id="container-table">
        ...
        <form class="w-full flex justify-between items-start mb-4 flex-col sm:flex-col"
            action="{{ route('admin.fleets.update', $fleet->fleet_ID) }}" method="post" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            {{-- Baris 1 --}}
            <div
                ...
                <div class="w-full">
                    <label for="fleet_nopol"
                        class="block w-fit mb-2 text-sm font-medium text-[#344357] dark:text-white">Nopol
                        Armada</label>
                    <div class="relative">
                        <input type="text" id="fleet_nopol" aria-describedby="helper-text-explanation"
                            ...
                            placeholder="B 9772 UXX" name="fleet_nopol" autocomplete="off" data-char-count maxlength="11"
                            value="{{ old('fleet_nopol', $fleet->fleet_nopol) }}">
                        @error('fleet_nopol')
                ...
                <div class="w-full">
                    ...
                    <div
                        ...
                        <select
                            ...
                            <option value="Van" {{ old('fleet_type', $fleet->fleet_type) == 'Van' ? 'selected' : '' }}>
                                Van
                            </option>
                            <option value="Pickup"
                                {{ old('fleet_type', $fleet->fleet_type) == 'Pickup' ? 'selected' : '' }}>Pickup
                            </option>
                            <option value="CDE Box"
                                {{ old('fleet_type', $fleet->fleet_type) == 'CDE Box' ? 'selected' : '' }}>CDE Box
                            </option>
                        </select>
            ...
            <div
                ...
                <div class="w-full">
                    ...
                    <div class="relative">
                        <input type="text" id="fleet_merk" aria-describedby="helper-text-explanation"
                            class="border-0 ring-1 ring-[#344357] text-[#344357] text-sm rounded-lg focus:ring-2 focus:outline-none block w-full p-2.5"
                            placeholder="Daihatsu Gran Max" name="fleet_merk" autocomplete="off" data-char-count
                            maxlength="100" value="{{ old('fleet_merk', $fleet->fleet_merk) }}">
                        @error('fleet_merk')
                            <p class="mt-2 text-sm text-red-500">
                                {{ $message }}
                            </p>
                ...
                <div class="w-full">
                    <label for="fleet_KIR_date"
                        ...
                        Terakhir</label>
                    <div class="relative">
                        <input id="fleet_KIR_date" datepicker datepicker-autohide type="text"
                            ...
                            placeholder="Pilih tanggal" name="fleet_KIR_date"
                            value="{{ old('fleet_KIR_date', $fleet_KIR_date) }}" autocomplete="off">
                    ...
                    <div
                        ...
                        <select
                            ...
                            <option value="TERSEDIA"
                                {{ old('fleet_status', $fleet->fleet_status) == 'TERSEDIA' ? 'selected' : '' }}>Tersedia
                            </option>
                            <option value="DIGUNAKAN"
                                {{ old('fleet_status', $fleet->fleet_status) == 'DIGUNAKAN' ? 'selected' : '' }}>Digunakan
                            </option>
                            <option value="PERBAIKAN"
                                {{ old('fleet_status', $fleet->fleet_status) == 'PERBAIKAN' ? 'selected' : '' }}>Perbaikan
                            </option>
                        </select>
                        ...
                        <select
                            ...
                            @foreach ($couriers as $courier)
                                <option value="{{ $courier->courier_ID }}"
                                    {{ old('courier_ID', $fleet->courier_ID) == $courier->courier_ID ? 'selected' : '' }}>
            ...
            <div
                ...
                <div class="w-full box-border">
                    <label class="block w-fit mb-2 text-sm font-medium text-[#344357] dark:text-white"
                        for="fleet_img">Foto Armada<br>(Scan foto depan, belakang, kanan, kiri dan bersama kurir dalam 1
                        PDF)</label>
                    <div class="relative ring-1 ring-[#344357] rounded-lg focus:ring-2 overflow-hidden">
                        <input
                            ...
                            aria-describedby="fleet_img_help" id="fleet_img" type="text" accept="application/pdf"
                            value="{{ basename($fleet->fleet_img) }}">
                        <button type="button"
                    ...
                    <div class="relative ring-1 ring-[#344357] rounded-lg focus:ring-2 overflow-hidden mt-5">
                        <input
                            ...
                            aria-describedby="fleet_img_help" id="fleet_img" type="file" name="fleet_img"
                ...
                <div class="w-full box-border">
                    ...
                    <div class="relative ring-1 ring-[#344357] rounded-lg focus:ring-2 overflow-hidden">
                        <input
                            class="border-0  text-[#344357] focus:outline-none block w-full px-2 box-border text-base font-semibold cursor-not-allowed"
                            aria-describedby="fleet_docs_help" id="fleet_docs" type="text" accept="application/pdf"
                            value="{{ basename($fleet->fleet_docs) }}">
                        <button type="button"
                            class="absolute top-0 right-0 bg-yellow-300 text-black px-2 h-full cursor-pointer active:scale-105"
                    ...
                    <div class="relative ring-1 ring-[#344357] rounded-lg focus:ring-2 overflow-hidden mt-5">
                        <input
                            class="border-0  text-[#344357] text-sm  focus:outline-none block w-full px-2 box-border upload-pdf"
                            aria-describedby="fleet_docs_help" id="fleet_docs" type="file" name="fleet_docs"
                            accept="application/pdf">
                        <button type="button"
                            class="absolute top-0 right-0 bg-yellow-300 text-black px-2 h-full hidden cursor-pointer active:scale-105"
                            id="showPDF">
