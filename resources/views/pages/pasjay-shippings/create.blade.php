...
@section('content')
    <div class="bg-white shadow rounded-2xl p-6 flex flex-col mt-[20px]" id="container-table">
        ...
        <form class="w-full flex justify-between items-start mb-4 flex-col sm:flex-col"
            action="{{ route('admin.pasjay-shippings.store') }}" method="post" enctype="multipart/form-data">
            @csrf
            {{-- Baris 1 --}}
            <div
                ...
                <div class="w-full">
                    ...
                    <div class="relative">
                        <input type="number" id="rit" aria-describedby="helper-text-explanation"
                            class="border-0 ring-1 ring-[#344357] text-[#344357] text-sm rounded-lg focus:ring-2 focus:outline-none block w-full p-2.5"
                            placeholder="Ritase" name="rit" autocomplete="off" value="{{ old('rit') }}">
                        @error('rit')
                            <p class="mt-2 text-sm text-red-500">
                                {{ $message }}
                            </p>
            ...
            <div
                ...
                <div class="w-full">
                    <label for="created_at"
                        ...
                        Pengiriman</label>
                    <div class="relative">
                        <input id="created_at" datepicker datepicker-autohide datepicker-format="yyyy-mm-dd" type="text"
                        ...
                        <select
                            ...
                            @foreach ($locations as $location)
                                <option value="{{ $location->shploc_ID }}"
                                    {{ old('shploc_ID') == $location->shploc_ID ? 'selected' : '' }}>
