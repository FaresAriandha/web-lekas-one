...
@section('content')
    <ul class="text-sm font-medium text-center overflow-hidden rounded-lg shadow-md flex ">
        <li class="w-full focus-within:z-10">
            <a href="{{ route('admin.courier-payments.index', ['display_data' => 'paxel']) }}"
        ...
        <li class="w-full focus-within:z-10">
            <a href="{{ route('admin.courier-payments.index', ['display_data' => 'pasjay']) }}"
    ...
    <div class="bg-white shadow rounded-2xl p-6 mt-[20px] relative" id="container-table">
        ...
        <div class="flex justify-between items-start sm:items-start mb-4 flex-col sm:flex-col">
            <h2 class="text-xl font-semibold mb-10">Daftar Riwayat Pencairan Kurir</h2>
            <div class="flex flex-col sm:flex-row items-start flex-wrap justify-between w-full ">
                <form class="flex flex-col sm:flex-row items-start sm:items-center flex-wrap" method="get"
                    action="{{ route('admin.courier-payments.index') }}">
                    ...
                    <div class="relative max-w-sm mb-[10px] sm:mb-0 sm:mr-[10px]">
                        <input id="datepicker-autohide" datepicker datepicker-autohide datepicker-format="yyyy-mm-dd"
                            ...
                            placeholder="Pilih tanggal" name="payment_date" autocomplete="off"
                            value="{{ isset($payment_date) ? $payment_date : '' }}">
                        ...
                        @foreach ($courier_payments as $index => $courier_pay)
                            <tr class="bg-white border-b border-gray-200 hover:bg-gray-100 text-center">
                                <th scope="row" class="px-6 py-4 font-medium text-center">
                                    {{ $courier_payments->firstItem() + $index }}
                                </th>
                                <td class="px-6 py-4">
                                    {{ $courier_pay->created_at->format('d/m/Y') }}
                                </td>
                                <td class="px-6 py-4 w-[200px]">
                                    Rp. {{ number_format($courier_pay->total_charge_sum, 0, '.', ',') }}
                                </td>
                                <td class="px-6 py-4">
                                    Rp. {{ number_format($courier_pay->total_paid_sum, 0, '.', ',') }}
                                </td>
                                <td
                                    class="px-6 py-4 font-semibold {{ $courier_pay->total_charge_sum - $courier_pay->total_paid_sum < 200000 ? 'text-red-400' : '' }}">
                                    Rp.
                                    {{ number_format($courier_pay->total_charge_sum - $courier_pay->total_paid_sum, 0, '.', ',') }}
                                </td>
                                <td class="px-6 py-4 flex justify-center items-center">
                                    <div class="relative w-fit h-fit">
                                        ...
                                        <div
                                            class="menu-popup absolute {{ $courier_payments->firstItem() + $index == $courier_payments->lastItem() ? 'bottom-0' : '-top-[10px]' }} -left-[140px] hidden sm:right-36 w-32 bg-white shadow-lg rounded-md z-50 p-2 text-nowrap text-center">
                                            <a href="{{ route('admin.courier-payments.edit', ['date' => $courier_pay->created_at->format('Y-m-d'), 'display_data' => $display_data]) }}"
            ...
            <div class="justify-between items-center mt-6 flex">
                <div>
                    <h1 class="text-xs sm:text-base">
                        Showing {{ $courier_payments->firstItem() }} to {{ $courier_payments->lastItem() }} of
                        {{ $courier_payments->total() }}
                        entries
                    </h1>
                </div>
                <div class="space-x-1">
                    @if ($courier_payments->currentPage() > 1)
                        <a href="{{ $courier_payments->previousPageUrl() }}"
                    ...
                    @if ($courier_payments->hasMorePages())
                        <a href="{{ $courier_payments->nextPageUrl() }}"
                            class="px-4 py-2 ring-1 ring-black rounded-lg hover:bg-gray-100 cursor-pointer">
